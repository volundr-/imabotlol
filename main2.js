(function(self, $) {
    ...
    function draw() {
        ...
        if (0 != closingAnimationTime) {
            ...
            var bestText = "Score: " + ~~(closingAnimationTime / 100) + " Best: " + bestRound + " Best Time: " + bestTime / 1000;
            button.setValue(bestText);
            d = button.render();
            w = d.width;
            /** @type {number} */
            ctx.globalAlpha = 0.2;
    ...
    /** Debugging functions **/
    function get(k){return eval(k)}
    function set(k, v){return eval(k + ' = ' + JSON.stringify(v))};
    ...
    function getBiggestMe() {
        var me;
        _.each(getMe(), function(m){if(!me || m.nSize > me.nSize) me = m});
        return me;
    }
    ...
    function drawPoint(x_1, y_1, drawColor) {
        if (true) {
            var x1 = ((x_1 - l) * ratio) + width/2;
            var y1 = ((y_1 - b) * ratio) + height/2;
    ...
    function createSamplePoints() {
        var n = 3,
            me = getBiggestMe();
        if (!me) return;
        if (Math.abs(me.size - lastSampledSize) < 20) {
            return;
        }

        var resolution = me.size * 2,
            pos,
            last, point;
        samplePoints = [];
        sampleHeads = [];
        ...
        for(var a = 0; a < Math.PI * 2; a+= Math.PI / 10) {
            last = null;
            for(var i = n; i > 0; i--) {
                pos = calculatePosition(a, i * resolution, [0, 0]);
    ...
    function findBestDirection() {
        ...
        autoPilotX = (me.x + x + autoPilotX + autoPilotX) / 3;
        autoPilotY = (me.y + y + autoPilotY + autoPilotY) / 3;
    }
    ...
    function calculateRisk(x, y, me) {
        ...
        _.each(getOthers(), function(other) {
            ...
            risk += (Cr * other.size) / (1 + computeDistance(x, y, other.x, other.y))
        });
        _.each(arr, function(other) {
            Cr = getRiskConstant(me, other, x, y);
            risk += (Cr * other.size) / (1 + computeDistance(x, y, other.x, other.y))
        });
        return risk;
    ...
    var patchedTriggerObjectAt = triggerObjectAt;
    triggerObjectAt = function() {
        if (!toggles.autoPilot) {
            patchedTriggerObjectAt();
        } else {
            value = autoPilotX;
            x = autoPilotY;
        }
    ...
    var patchedBuild = build;
    build = function() {
        patchedBuild();
        forceRatio();
    };

    var patchedProcessData = processData;
